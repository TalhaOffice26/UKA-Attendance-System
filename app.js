...
// MongoDB-তে সেশন সংরক্ষণের স্কিমা
const authSchema = new mongoose.Schema(
  { _id: String, value: String },
  { collection: 'whatsapp_auth', versionKey: false }
);
...
// ফোন নম্বরকে Baileys JID (@s.whatsapp.net) ফরম্যাটে রূপান্তর
function formatToWhatsAppId(phone) {
  if (!phone) return null;
  let cleaned = phone.toString().replace(/[^0-9]/g, '').trim();
  if (cleaned.startsWith('880')) {
...
// সংখ্যাকে বাংলায় রূপান্তর করার হেল্পার ফাংশন
...
// ১০. হাজিরা সংরক্ষণ, অভিভাবকদের অ্যালার্ট ও প্রিন্সিপালকে ক্লাস সামারি প্রেরণ
app.post('/attendance/save', isAuthenticated, async (req, res) => {
  try {
    const { batch_id, class_number, class_date, attendance, send_whatsapp } = req.body;
    const batch = await Batch.findById(batch_id);
      ...
      if (shouldSend) {
        enqueueWaJob(async () => {
          try {
            ...
            // প্রিন্সিপাল স্যারের সামারির জন্য লিস্ট
            const presentList = [];
            const absentList = [];

            // ১. প্রতিটি শিক্ষার্থীর অভিভাবককে নোটিফিকেশন প্রদান
            for (let student of students) {
              const status = attendance[student._id.toString()];
              const cleanName = (student.name || '').trim();

              if (status === 'Present') {
                presentList.push(cleanName);
              } else if (status === 'Absent') {
                absentList.push(cleanName);
              }
            ...
            if (principalPhone) {
              ...
              if (principalWaId) {
                const totalCountBn = toBengaliDigits(students.length);
                const presentCountBn = toBengaliDigits(presentList.length);
                const absentCountBn = toBengaliDigits(absentList.length);

                const presentText = presentList.length > 0
                  ? presentList.map((name, i) => `${toBengaliDigits(i + 1)}. ${name}`).join('\n')
                  : '_কেউ উপস্থিত নেই_';

                const absentText = absentList.length > 0
                  ? absentList.map((name, i) => `${toBengaliDigits(i + 1)}. ${name}`).join('\n')
                  : '_কেউ অনুপস্থিত নেই_';

                const summaryMsg =
`📊 *দৈনিক হাজিরা রিপোর্ট - UKA Technical Institute*
━━━━━━━━━━━━━━━━━━━
🎓 *ব্যাচ:* ${batch.name}
📚 *ক্লাস:* Class ${classNum}
📅 *তারিখ:* ${class_date}
👨‍🏫 *শিক্ষক:* ${batch.teacherName || 'Not Assigned'} ${batch.teacherPhone ? `(${batch.teacherPhone})` : ''}
━━━━━━━━━━━━━━━━━━━
👥 *মোট শিক্ষার্থী:* ${totalCountBn} জন
✅ *উপস্থিত:* ${presentCountBn} জন
❌ *অনুপস্থিত:* ${absentCountBn} জন
🟢 *উপস্থিত শিক্ষার্থী:*
${presentText}
🔴 *অনুপস্থিত শিক্ষার্থী:*
${absentText}
━━━━━━━━━━━━━━━━━━━UKA Attendance Automation System`;

                console.log(`📨 Sending Attendance Summary to Principal Sir (${principalWaId})...`);
                await sendWhatsAppAlert(principalWaId, summaryMsg, 'Principal Sir');
              } else {
                console.warn('⚠️ Invalid Principal phone number format.');
              }
            }
